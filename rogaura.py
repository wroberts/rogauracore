...
def rainbow():
    return multi_static(Color.fromhexstr('ff0000'), Color.fromhexstr('ffff00'), Color.fromhexstr('00ffff'), Color.fromhexstr('ff00ff'))
...
def parseargs(argv):
    ...
    if argv:
        name = argv[0]
    if not argv or '-?' in argv or '-h' in argv or '--help' in argv or name not in mapping:
    ...
    for argname, value in zip(argnames, argv[1:]):
        if argname.startswith('color'):
            ...
            raise Exception('Undecipherable argument name {} in function {}'.format(argname, name))
